    ...
    (function() {
        ...
        async function sendOtp() {
            ...
            if (currentUser && currentUser.email && email.toLowerCase() !== currentUser.email.toLowerCase()) {
                emailInput.focus();
                emailInput.style.borderColor = '#d62828';
                errSpan.textContent = 'Email tidak sesuai dengan akun Master Admin yang sedang login.';
                errDiv.style.display = 'block';
                return;
            }
            ...
            try {
                ...
                if (res.status === 429) {
                    ...
                    Swal.fire({ icon: 'warning', title: 'Mohon Tunggu', text: data.message, confirmButtonColor: '#d62828' });
                    return;
                }
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Kirim Kode OTP';
                Swal.fire({ icon: 'error', title: 'Gagal', text: 'Gagal mengirim kode OTP. Periksa koneksi Anda.', confirmButtonColor: '#d62828' });
            }
        }
            ...
            document.getElementById('resendOtp').addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (res.status === 429) {
                        ...
                        Swal.fire({ icon: 'success', title: 'Terkirim!', text: 'Kode OTP baru telah dikirim.', timer: 2000, showConfirmButton: false });
                        seconds = otpExpirationMinutes * 60;
                        startCountdown(seconds);
                    }
                } catch(err) {
                    Swal.fire({ icon: 'error', title: 'Gagal', text: 'Gagal mengirim ulang.', confirmButtonColor: '#d62828' });
                }
        ...
        async function verifyOtpStep() {
            ...
            try {
                const res = await fetch(`${API}/admin/verify-otp`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
                    body: JSON.stringify({ email: adminEmail, otp_code: otp })
                });
                const data = await res.json();
                ...
                if (!res.ok) {
                    ...
                    if (res.status === 429 && data.remaining_attempts === undefined) {
                        ...
                        Swal.fire({ icon: 'error', title: 'OTP Tidak Valid', text: data.message, confirmButtonColor: '#d62828' });
                        document.getElementById('otpInput').value = '';
                        document.getElementById('otpInput').focus();
                    } else {
                        Swal.fire({ icon: 'error', title: 'Gagal', text: data.message || 'Verifikasi OTP gagal.', confirmButtonColor: '#d62828' });
                    }
                    return;
                }
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="fa-solid fa-check-circle"></i> Verifikasi';
                Swal.fire({ icon: 'error', title: 'Gagal', text: 'Terjadi kesalahan. Periksa koneksi Anda.', confirmButtonColor: '#d62828' });
            }
        }
